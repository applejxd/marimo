...
def _():
    ...
    空間刻みを $h=0.005$ と細かく取るのは、初期波形の 1 山が格子 50 点で表現され、
    反射のたびに現れる数値分散が目に見えないようにするためである。
...
def _():
    ...
    格子を $50\times50$ と細かく取るのは、円形に広がる波面が階段状に見えるのを
    避けるためである。$20\times20$ 程度では、波面が格子の向きに沿って角張る。
    時間は、壁での反射が 2 回起きる $t=1.5$ までとする。
    """)
    return
