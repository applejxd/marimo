...
def _():
    ...
    有限要素法（FEM）を 2 通りのやり方で扱います
    （`legacy/simulation/FEM.ipynb` を marimo 向けに整理したものです）。
    ...
    ためのものです。線形の 1 次要素（P1）を周期的な 1 次元メッシュ上に並べ、
    Zabusky と Kruskal が 1965 年にソリトンを発見したときと同じ初期値問題を
    再現します。後半は実用的な FEM ライブラリ NGSolve で 2 次元問題を解きます。

    NGSolve の可視化ユーティリティ `ngsolve.webgui.Draw` は WebGL のウィジェットを
    返すため、静的 HTML へ書き出すと空欄になってしまいます。この notebook では
    すべての描画を matplotlib で行い、HTML でも図が残るようにしています。
    """)
    return
...
def _():
    ...
    $\int u_{xxx}φ_i\,dx=\int wφ_i'\,dx=(Bw)_i$ と 2 段階で評価します。
    非線形項は 1 次要素で厳密に積分でき、
    $$
    \int u\frac{∂u}{∂x}φ_i\,dx=\frac{(u_{i+1}+u_i+u_{i-1})(u_{i+1}-u_{i-1})}{6}
    $$
    となります。まとめると、解くべき常微分方程式は
    $$
...
def _():
    ...
    もとの設定は $h=0.02$、$t_{\max}=0.4$（保存 11 コマ）でした。$t=0.4$ は
    波が切り立ち始めた直後にすぎず、この方程式で本当に見たいソリトンの生成も
    再帰現象も始まる前に終わっていました。

    時間の目安は破断時刻 $t_B=1/\pi\simeq0.318$ です。ソリトン列が出そろうのが
    $t\simeq3.6\,t_B\simeq1.15$、初期波形へ戻る再帰時刻が
    $t_R\simeq30.4\,t_B\simeq9.7$ なので、$t_{\max}=10$ まで積分すれば
    「切り立ち → ソリトン生成 → 再帰」が 1 本のアニメーションに収まります。
    格子も $h=0.01$ へ細かくして、幅が $δ$ 程度しかないソリトンを解像します。
    """)
    return
...
def _():
    ...
    オフセットを渡して組み立てます。オフセット $\pm(N-1)$ の項が、右端と左端を
    つなぐ周期境界条件に対応します。

    実際に使うのは $200\times200$ の行列なので、構造が読めるよう
    まず $N=6$、$h=1$ の小さな例で 3 つを並べて確認します。
    """)
    return
...
def _():
    ...
    `np.roll` で隣の節点をずらして取り出せば 1 行で書けます。
    `np.roll` が配列の端を巻き戻すので、これがそのまま周期境界条件になります。

    下の例では $u=(1,2,3)$ に対して中央の成分が
    $(3+2+1)(3-1)/6=2$ となることを確認しています。
    """)
    return
...
def _():
    ...
    連立方程式を 1 回の評価あたり 2 回解く必要があります。$M$ は時間に依らないので、
    毎回反復法で解き直す代わりに `splu` で一度だけ LU 分解し、
    以後は前進後退代入だけで済ませます。

    もとの実装は許容誤差 $10^{-5}$ の `bicg` を使っていました。今回は時間ステップが
    10 万回・右辺評価が 40 万回に増えるため、そのままでは反復法の打ち切り誤差が
    蓄積します。LU 分解なら誤差は丸め誤差の水準で、かつ計測では同じ問題を解くのに
    要する時間が 1/20 になります。

    なお $B$ は反対称なので、$Bw$ と $w^{\mathsf{T}}B$ は符号が逆になります。
    $\int u_{xxx}φ_i\,dx=\int wφ_i'\,dx=\sum_jw_j\int φ_jφ_i'\,dx=(Bw)_i$ なので、
    使うのは `kdv_gradient @ w` の側です。
    """)
    return
...
def _():
    ...
    初期条件は周期区間 $[0,2)$ 上の $\cos(\pi x)$ です。
    時間積分は古典的な 4 次のルンゲ＝クッタ法（RK4）を固定刻み $Δt=10^{-4}$ で
    回します。分散項の固有値は純虚数なので、虚軸方向に安定領域を持つ RK4 が
    適しています（前進オイラー法は虚軸上で必ず発散します）。
    """)
    return
...
def _():
    ...
    そこから生まれた複数の筋がそれぞれ一定の傾きで進んでいく様子が見えます。
    傾きの逆数が伝播速度で、色が濃い（振幅が大きい）筋ほど速く進みます。
    これが「振幅が大きいソリトンほど速い」という KdV の関係です。
    """)
    return
...
def _():
    ...
    ライブラリで、弱形式を数式に近い形で書くとアセンブルまで自動で行ってくれます。
    このリポジトリでは `specialized` 依存グループに含まれるため、
    `uv sync --all-groups` で導入されます（Colab 用の初期化セルは使いません）。

    NGSolve に付属する `Draw` は WebGL のウィジェットを返し、marimo の静的 HTML では
    描画されません。そこで、メッシュの三角形分割を matplotlib の
    `Triangulation` に移し替えるヘルパーと、`CoefficientFunction` や
    `GridFunction` を格子上でサンプリングするヘルパーを用意します。
...
def _():
    mo.md(r"""
    まずメッシュを生成します。`maxh` は要素の最大辺長で、小さくするほど
    細かいメッシュになります。もとの `maxh=0.2` から `maxh=0.12` へ細かくして、
    等高線が角張って見えないようにしています。
    """)
    return
...
def _():
    mo.md(r"""
    次に有限要素空間を定義します。`H1` は連続な区分多項式の空間で、
    `order=2` は 2 次要素（各辺の中点にも自由度が付く）を意味します。
    境界条件もここで指定し、`dirichlet="bottom|right"` とした辺の自由度は
    後の `FreeDofs()` から除かれて値が固定されます。指定しなかった上辺・左辺には
    何もしなくても自然境界条件（$∂u/∂n=0$）が課されます。
    """)
    return
...
def _():
    ...
    \end{aligned}
    $$
    となります。ディリクレ境界では $v=0$、ノイマン境界では $∂u/∂n=0$ なので、
    1 つ目の表面項は消えます。残るのは
    $$
    a(u,v)=\int\nabla u\cdot\nabla v\,dS,\quad
    \ell(v)=\int fv\,dS
    $$
    で、これがそのまま `BilinearForm` と `LinearForm` に対応します。
    """)
    return
...
def _():
    ...
    組み上がった右辺ベクトルと左辺行列を NumPy / SciPy へ取り出せます。
    左辺の疎行列は、節点の番号付けを反映した対称なパターンになります。
    """)
    return
...
def _():
    ...
    描きます。下辺と右辺で $u=0$ に固定され、$f=x$ に押し上げられた分布が
    左上に寄っている様子が確認できます。
    """)
    return
...
def _():
    ...
    $$
    を正方領域 $[-1,1]^2$ で解きます。$α=0.01$ は熱の拡散しやすさ、
    $b$ は温度を運ぶ「風」です。全周をディリクレ境界とします。
    ...
    $$
    で進めます。この形の利点は、拡散項による強い剛性があっても
    時間刻みを安定性ではなく精度だけで選べることです。
    実装では等価な増分形
    $$
    u^{n+1}=u^n+(M+Δt\,A)^{-1}\left(Δt\,f-Δt\,Au^n\right)
    $$
    を使い、$(M+Δt\,A)^{-1}$ を最初に 1 回だけ作って使い回します。

    もとの設定は `maxh=0.25`、$Δt=10^{-3}$、$t_{\max}=2$ で保存 11 コマでした。
    ここでは `maxh=0.15`、$Δt=5\times10^{-4}$、$t_{\max}=4$ の 81 コマにします。
    風の角速度は領域中央で約 2 なので、1 回転に要する時間は $\pi\simeq3.14$ です。
...
def _():
    ...
    $$
    です。境界上で法線成分がゼロになるので、風は領域から出ていきません。
    原点のまわりを時計回りに回る渦になっており、中心付近では
    $b\simeq(2y,-2x)$、つまり角速度 2 の剛体回転です。
    """)
    return
...
def _():
    mo.md(r"""
    双一次形式 $a(u,v)=\int(α\nabla u\cdot\nabla v+(b\cdot\nabla u)v)\,dS$ と
    質量形式 $m(u,v)=\int uv\,dS$ を組み立てます。移流項があるため $a$ は
    対称ではありません（`symmetric=False`）。
    """)
    return
...
def _():
    mo.md(r"""
    $M^\ast=M+Δt\,A$ を作って逆行列（実体は疎行列の直接ソルバー）を用意します。
...
def _():
    ...
    「熱の湧き出しと吸い込み」の対にします。この $f$ は時間に依らないので、
    最終的には風による輸送・拡散・湧き出しが釣り合った定常状態へ落ち着きます。
    """)
    return
...
def _():
    mo.md(r"""
    初期条件は $u(x,y,0)=(1-y^2)x$ です。左半分が冷たく右半分が暖かい分布で、
    境界上の値がそのままディリクレ境界条件として保持されます。
    """)
    return
...
def _():
    ...
    $96\times96$ の格子へサンプリングして 81 コマぶんの配列を作ります。
    可視化用のサンプリングを積分ループの中で済ませておくと、
    大量の `GridFunction` を保持せずにアニメーションを作れます。
    """)
    return
...
def _():
    ...
    81 コマを 1 コマ 100 ms で再生するので、再生時間は約 8.1 秒です。
    色は $0$ を白とする発散カラーマップで、範囲を全コマ $[-1,1]$ に固定します。
    """)
    return
...
def _():
    ...
    要素 1 個あたりでも $|b|h/(2α)\simeq2\times0.15/0.02=15$ と移流が優勢です。
    標準ガレルキン法はこの条件で数値振動を起こしやすいのですが、
    3 次要素（`order=3`）を使っているため実効的な格子が細かく、
    実際に解の値は初期条件と同じ $[-1,1]$ の範囲に収まり
    （最終出力の `u の範囲` を参照）、目立った振動は生じていません。
    """)
    return
