...
def _():
    ...
    時間の目安は破断時刻 $t_B=1/\pi\simeq0.318$ である。移流項だけなら
    この時刻で勾配が無限大になるが、分散項があるとその手前でソリトンが生まれる。
    ソリトン列が出そろうのが $t\simeq3.6\,t_B\simeq1.15$、初期波形へ戻る再帰時刻が
    $t_R\simeq30.4\,t_B\simeq9.7$ なので、$t_{\max}=10$ まで積分すれば
    「切り立ち → ソリトン生成 → 再帰」が 1 本のアニメーションに収まる。

    格子は、幅が $δ$ 程度しかないソリトンを解像できるよう $h=0.01$ とする。
    $h=0.02$ ではソリトン 1 個が格子数個で表現され、頂点が角張って見える。
    """)
    return
...
def _():
    ...
    反復法（`bicg` など）でも解けるが、この問題では時間ステップが 10 万回・
    右辺評価が 40 万回に達するため、打ち切り誤差が蓄積しうる。LU 分解なら
    誤差は丸め誤差の水準にとどまり、計測では同じ問題を解くのに要する時間が
    許容誤差 $10^{-5}$ の `bicg` の 1/20 になる。
...
def _():
    ...
    細かいメッシュになる。ここでは等高線が角張って見えないよう `maxh=0.12` とする。
    """)
    return
...
def _():
    ...
    離散化は `maxh=0.15`、$Δt=5\times10^{-4}$、$t_{\max}=4$ とし、81 コマを保存する。
