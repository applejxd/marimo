...
def _():
    ...
    空間刻みを $h=0.005$ と細かく取るのは、衝撃波の厚みが
...
def _():
    ...
    格子点数は、3 階微分を解像するために大きく取る必要がある。$n_x=200$ では
    $t\simeq3.17$ で `solve_ivp` が
    `Required step size is less than spacing between numbers.` を返して
    積分が破綻する。しかも `solve_ivp` は失敗しても例外を投げないため、
    `sol.success` を確認せずに `dense_output` から値を取り出すと、
    発散した外挿値がそのまま図になる。**このセルでは `sol.success` を必ず検査する。**
