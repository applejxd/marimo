...
def _():
    ...
    有限差分で空間を離散化し `scipy.integrate.solve_ivp` で時間積分します
    （`legacy/simulation/ParabolicPDE.ipynb` を marimo 向けに整理したものです）。

    扱う 3 つの方程式は、線形の拡散から非線形・分散性へ段階的に難しくなります。
...
def _():
    ...
    \frac{∂u}{∂t}=D\nabla^2u
    $$
    です。ここでは 1 次元で、長さと時間を適切に規格化して $D=1$ として解きます。

    初期条件をデルタ関数 $u(x,0)=δ(x)$ に取ると、解は解析的に求まり
    $$
    u(x,t)=\frac{1}{\sqrt{4\pi Dt}}\exp\left(-\frac{x^2}{4Dt}\right)
    $$
    となります（熱核、あるいはグリーン関数）。数値解をこの厳密解と重ねて描くことで、
    離散化と時間積分がどれだけ正確かをその場で確認できます。

    計算領域は $x\in[-2,2]$、空間刻みは $h=0.01$（格子点 400 個）とします。
...
def _():
    ...
    $$
    で置き換えます。境界は、十分遠方では $u$ が $0$ に減衰するという想定のもと、
    ディリクレ条件
    $$
    u(x_0)=u(x_N)=0
    $$
    を課します。$u$ を適切に平行移動して定義し直せば、境界値は常にこの形にできます。

    次のセルの `derivative_matrices` は、この境界条件に対応する 1 階微分行列
    $(S_{+}-S_{-})/2h$ と 2 階微分行列 $(S_{+}-2I+S_{-})/h^2$ を組で返します
    （1 階微分は後のバーガース方程式で使います）。両端の行では領域外を指す
    シフト成分を落としており、それがディリクレ条件に対応します。

    構造が読み取れるよう、まず $n=6$、$h=1$ の小さな例を表示します。
    """)
    return
...
def _():
    mo.md(r"""
    デルタ関数を格子上で表現するには、中央の 1 点だけに $1/h$ を置きます。
    こうすると $\sum_iu_ih=1$ となり、全体の積分が $1$ に規格化されます。
    今回は $h=0.01$ なので高さは $100$ です。

    [数値解の不安定性より、時間方向の刻み幅 $k$ は空間方向の刻み幅 $h$ に対して
    十分に小さく取る必要があります](http://www.nibb.ac.jp/miyakohp/asari/htdocs/?page_id=60)。
    具体的には
    $$
    \frac{Dk}{h^2}\leq\frac{1}{2}
    $$
    で、これは差分化した係数行列の固有値を陽解法の安定領域へ収める条件です。
    $h=0.01$、$D=1$ なら $k\leq5\times10^{-5}$ となり、アニメーションのコマ間隔
    $Δt_{\text{frame}}=10^{-3}$ よりずっと小さくなります。適応刻みの `RK45` は
...
def _():
    ...
    `sol.success` の確認は必須です。刻み幅が下限に達して積分が途中で
    打ち切られても `solve_ivp` は例外を投げないため、確認を省くと
    発散した結果をそのまま可視化してしまいます。
    """)
    return
...
def _():
    ...
    各コマで数値解（実線）と熱核の厳密解（破線）を重ねます。
    デルタ関数の高さ $1/h=100$ は縦軸に収まらないので、$t=0.004$
    （厳密解のピークが $1/\sqrt{4\pi t}\simeq4.46$）から描き始めます。

    97 コマを 1 コマ 80 ms で再生するので、再生時間は約 7.8 秒です。
    """)
    return
...
def _():
    ...
    収まります。分布の幅は $\sqrt{2Dt}$ で広がり、ピークは $1/\sqrt{4\pi Dt}$ で
    下がるため、$t$ が 4 倍になるとピークは半分になります。

    ここが双曲型との決定的な違いです。波動方程式では波形が形を保って伝わりましたが、
    拡散方程式では高周波成分ほど速く減衰し（波数 $k$ の成分は $e^{-Dk^2t}$）、
    どんな初期分布も最終的に滑らかなガウス分布へ近づきます。
    """)
    return
...
def _():
    ...
    $$
    は、移流項 $-uu_x$ と拡散項 $Du_{xx}$ の競合を最小構成で表した方程式です。
    移流項は「値が大きいところほど速く動く」ため波形を切り立たせ、
    拡散項はそれを鈍らせます。両者が釣り合った定常構造が衝撃波です。
    ...
    $\sim D/\max|u|=0.01$ 程度しかないためです。$h=0.02$ では衝撃波が
    格子 1 個分より薄くなり、切り立った部分に非物理的な振動が出ます。
    $h=0.005$ なら遷移層に 2〜4 点が入り、滑らかに解像されます。
    """)
    return
...
def _():
    mo.md(r"""
    右辺は拡散方程式と同じ差分行列から組み立てます。異なるのは移流項が
    `-u * (first @ u)` という $u$ の二次形式になっている点だけで、
    ここが非線形性の出どころです。行列とベクトルの積が毎回必要になるため、
    右辺の評価回数は拡散方程式より増えます。

    出力は $t=0$ における 2 つの項の大きさです。移流項が $\pi\simeq3.14$、
    拡散項が $4\pi^2D\simeq0.39$ で、初期状態では移流項が 1 桁大きく、
    波形を切り立たせる働きが優勢だと分かります。切り立って $u_{xx}$ が
    大きくなると拡散項が追いつき、そこで衝撃波の厚みが決まります。
    """)
    return
...
def _():
    ...
    等値線が集まり、そこから上へ細い界面がまっすぐ伸びる様子が見えます。
    この界面が衝撃波です。
    """)
    return
...
def _():
    ...
    201 コマを 1 コマ 50 ms で再生するので、再生時間は約 10 秒です。
    """)
    return
...
def _():
    ...
    $$
    を初期値問題として解きます。空間方向は周期境界条件です。

    バーガース方程式の拡散項 $Du_{xx}$ が、ここでは分散項 $-u_{xxx}$ に
    置き換わっています。拡散項がエネルギーを散逸させたのに対し、分散項は
    エネルギーを保存したまま波数ごとに位相速度を変えます。その結果、
    切り立った波面は「鈍る」のではなく**振動する列に分解**され、
    最終的に振幅と速度が対応した孤立波（ソリトン）の列になります。
    ...
    もとの設定は $n_x=200$、$t\leq4$ でしたが、この格子では分散項が解像できず、
    $t\simeq3.17$ で `solve_ivp` が
    `Required step size is less than spacing between numbers.` を返して
    積分が破綻していました。`sol.success` を確認していなかったため、
    アニメーションの後半には発散した値が描かれていました。
    $n_x=512$ にすると最後まで安定に積分でき、$n_x=1024$ で計算した場合と
    ソリトンの本数も位置も一致します（尖頭の高さだけは格子に依存し、
    $n_x=512$ で 2.97、$n_x=1024$ で 2.83 と数 % 変わります）。
    """)
    return
...
def _():
    ...
    隣接点を指す行列が作れます。左端の 1 つ手前が右端になります。
    3 階微分には 5 点ステンシル
    $$
    \frac{∂^3f}{∂x^3}(x_i)\simeq\frac{f_{i+2}-2f_{i+1}+2f_{i-1}-f_{i-2}}{2h^3}
    $$
    を使います。$1/h^3$ という因子のため、$h$ を半分にすると係数は 8 倍になり、
    陽解法が必要とする時間刻みも急激に小さくなります。
    これが KdV の数値計算を重くしている理由です。
    """)
    return
...
def _():
    mo.md(r"""
    右辺 $-6uu_x-u_{xxx}$ を関数として定義します。`solve_ivp` の `args` を通じて
...
def _():
    ...
    刻み幅の上限を `max_step` で縛らず、`RK45` の適応制御に任せます。
    5 点ステンシルの 3 階微分行列の固有値は純虚数で、絶対値は最大で
    $2.6/h^3\simeq3.5\times10^2$ に達します。ソルバーはこれに合わせて
    刻みを $10^{-3}$ 程度まで詰め、$t\in[0,10]$ の積分に約 3100 ステップを要します。

    結果は再利用できるよう `.npz` にも保存します
    （`.npz` は Git の追跡対象外です）。
    """)
    return
...
def _():
    ...
    保存した `.npz` を読み直してから描画します。こうしておくと、
    積分をやり直さずに図だけを作り直せます。
