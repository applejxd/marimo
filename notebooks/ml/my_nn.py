...
def _():
    ...
    ライブラリの自動微分に頼らず、順伝播と逆伝播を NumPy だけで実装して数字を分類します。
    レイヤは Sigmoid・Affine・Softmax-with-Loss の3種類で、それぞれ `forward` と
    `backward` を持ちます。`backward` を数式から導いて実装することがこの notebook の主眼です。

    元 notebook は OpenML の MNIST（`fetch_openml("mnist_784")`）と PyCaret を経由して
    いましたが、ここでは自前実装の順伝播・逆伝播を学ぶ意図を保ちつつ、
...
def _():
    ...
    整数値です。これを16で割って $[0, 1]$ に収め、64次元のベクトルとして入力します。

    ラベルは0から9の整数ですが、交差エントロピーの計算では正解クラスだけが1のベクトルが
    必要なので、`np.eye(10)[labels]` で one-hot 表現へ変換します。

    データは**学習・検証・テストの3つ**に分けます。まず75対25で学習用と保留分に分け、
    保留分をさらに半分ずつ検証用とテスト用にします。学習曲線の監視や初期値スケールの比較には
    検証データを使い、テストデータは最後に一度だけ使います。同じデータで何度も判断すると、
    その data に合わせ込んだ結果を汎化性能と取り違えるためです。
    分割はいずれも `stratify` で各クラスの比率を保ちます。
    """)
    return
...
def _():
    mo.md(r"""
    入力画像の先頭10枚を表示します。8×8という粗さでも数字が判別できることを確認します。
    """)
    return
...
def _():
    ...
    以降のレイヤはすべて**バッチを行方向に並べた規約**で書きます。入力 $X$ は
    $N \times D_{\text{in}}$ 行列で、$n$ 行目が $n$ 番目のサンプルです。教科書でよく見る
    $Y = WX + b$（サンプルを列に並べる規約）とは転置の関係にあるので、
    式に現れる行列の順序と転置の位置が入れ替わる点に注意してください。
...
def _():
    ...
    シグモイド関数と、その導関数が出力だけで書けることを使います。
    ...
    このレイヤは要素ごとの写像なので、入力がベクトルでも行列でも同じ式で書けます。
    $Y = \sigma(X)$ とすると、逆伝播は要素ごとの積（アダマール積）になります。
    ...
    `backward` は入力を持たなくても計算できます。入力は $N \times D$ の配列、
    出力は同じ形の配列です。
    """)
    return
...
def _():
    ...
    バッチを行に並べた規約では、順伝播は次の形になります。$W$ は
    $D_{\text{in}} \times D_{\text{out}}$、$b$ は $D_{\text{out}}$ 次元で、
    $b$ は全サンプルへ同じ値が足されます（ブロードキャスト）。
    ...
    $\partial L/\partial Y$ を $\mathrm{d}Y$ と書くと、連鎖律から3つの勾配が得られます。
    $W$ の勾配と $b$ の勾配はバッチ方向に和を取る形になります。
...
def _():
    ...
    softmax は指数関数を使うため、入力が大きいと `exp` があふれます。分子と分母に同じ
    定数を掛けても値が変わらない性質を使い、**行ごとの最大値**を引いてから指数を取ります。
    引いた後の最大の logit が0になるので、指数の最大値が $e^0=1$ に収まります。
    ...
    したがって損失 $-\log y$ が $+\infty$ へ発散します。softmax の出力を経由してから
    対数を取ると、小さい確率が丸めで0になった時点で計算が壊れます。logit から
    log-sum-exp を使って対数確率を直接求めれば、この丸めを経由しません。
    $a_{nk} - C_n \le 0$ なので指数はあふれず、対数の引数も1以上になります。
    バッチに対してはサンプルごとの値の平均を損失とします。
    ...
    引き算だけになることです。個別に微分すると商の微分が現れますが、合成すると打ち消し合います。
    損失をバッチ平均で定義したので、勾配も $N$ で割ります。微小量を足して対数を守る実装だと
    この式は近似になりますが、log-sum-exp で計算しているので厳密に成り立ちます。

    $$
    \frac{\partial L}{\partial A} = \frac{Y - T}{N}
    $$

    `forward` の入力は Affine の出力（logit）と one-hot の正解で、出力はスカラーの損失です。
    `backward` の戻り値は logit と同じ $N \times 10$ の配列です。
    """)
    return
...
def _():
    ...
    Affine → Sigmoid → Affine → Sigmoid → Affine の5層を `OrderedDict` に順番に積みます。
    64次元の入力を64、32と変換し、最後に10クラスの logit を出します。
    `predict` はこの順に `forward` を呼ぶだけ、`gradient` は逆順に `backward` を呼ぶだけです。

    **重みの初期値スケールは学習の進み方を大きく左右します。** ここでは Xavier の初期化を使い、
    層ごとに入力次元 $D_{\text{in}}$ に応じて標準偏差を変えます。
    ...
    レイヤを通したときの分散を揃える、という考え方です。この notebook の構成では
    $D_{\text{in}}=64$ の層で 0.125、$D_{\text{in}}=32$ の層で 0.177 になります。
    後の節で、これを定数 0.05 に固定した場合と比較します。

    実装上の注意が1つあります。`self.layers` の各 Affine は `self.params` の配列を
    **同じオブジェクトとして**参照しています。そのため学習ループの
    `params[key] -= lr * grad[key]` という**その場での**更新はレイヤ側にも反映されます。
    ここを `params[key] = params[key] - lr * grad[key]` と書くと新しい配列に束ね直され、
    レイヤは古い配列を参照したままになり、まったく学習が進みません。
    """)
    return
...
def _():
    ...
    誤りに気付けないことがあります。そこで、解析的に求めた勾配を数値微分と突き合わせます。
    中心差分は次の式です。
    ...
    打ち切り誤差は $O(h^2)$ で減りますが、$h$ を小さくしすぎると引き算の桁落ちが効いて
    かえって悪化します。ここでは $h=10^{-5}$ を使います。

    全パラメータを数値微分すると重いので、各パラメータ配列から5箇所ずつ抜き出して比べます。
    抜き取り検査なので実装全体の正しさを証明するものではありませんが、
    誤差が十分小さければ実装が正しいという確信は大きく高まります。
    絶対誤差と相対誤差の両方を表示します。
    """)
    return
...
def _():
    ...
    確率的勾配降下法で学習します。毎回、学習データから重複なしで64件を選び、
    その勾配で全パラメータを更新します。学習率は0.5、反復回数は3,000回です。
    1,347件の学習データに対して 3000 × 64 / 1347 ≒ 143 エポックに相当します。

    $$
    \theta \leftarrow \theta - \eta \frac{\partial L}{\partial \theta}, \qquad \eta = 0.5
    $$

    初回と20回ごとに、そのミニバッチの損失と、学習データ・検証データ全体の正解率を記録します。
    損失は更新後に測っているので、その反復の結果を表します。
    比較の目安として、10クラスを当てずっぽうに答えるときの交差エントロピーは
    $\log 10 \approx 2.303$、正解率は0.1です。
    """)
    return
...
def _():
    mo.md(r"""
    記録の末尾5行です。損失が当てずっぽうの2.303から大きく下がり、
    学習データと検証データの正解率が揃って上がっていれば、学習は成功しています。
    """)
    return
...
def _():
    mo.md(r"""
    左はミニバッチ損失、右は正解率の推移です。ミニバッチ損失は64件だけで測るため
    上下に振れますが、全体の傾向は下降します。右の図で学習用と検証用の曲線が
    大きく離れていなければ、極端な過学習は起きていません。
    """)
    return
...
def _():
    ...
    初期化の話を実測で確かめます。同じ構成・同じ学習率で、重みの標準偏差だけを
    Xavier（$1/\sqrt{D_{\text{in}}}$）と定数 0.05 に変えて比較します。

    定数 0.05 は $D_{\text{in}}=64$ の層に対して Xavier の 0.125 の半分以下です。
    重みが小さいと、Affine の逆伝播 $\mathrm{d}X = \mathrm{d}Y\,W^{\top}$ で小さな $W$ が
    掛かるため、入口へ向かうほど勾配が縮みます。順伝播でも Affine の出力が0付近に集中し、
    シグモイドの出力が0.5付近に固まって、サンプルごとの差が出にくくなります。

    この条件で実際に効いたのは**収束の速さ**でした。反復を十分に取ればどちらも同程度まで
    学習しますが、初期の伸びが大きく違います。次のセルでは50反復ごとに検証正解率を測り、
    正解率0.9に最初に到達した反復と、学習途中の目安として600反復時点の正解率を比べます。
    ここでもテストデータは使いません。
    """)
    return
...
def _():
    mo.md(r"""
    曲線の立ち上がりの差がそのまま学習の速さの差です。定数0.05の側は序盤ほとんど
    当てずっぽうの水準（0.1）に留まり、後から追いついてきます。反復回数が足りないまま
    打ち切ると、実装が正しくても「学習できていない」という誤った結論になりかねません。
    初期値スケールは、モデルの表現力ではなく最適化の効きやすさを左右する設定です。
    """)
    return
...
def _():
    ...
    ここまでの判断はすべて検証データで行いました。最後に、一度も参照していない
    テストデータ225件で汎化性能を測ります。scikit-learn の `accuracy_score` を使うので、
    自前実装の `network.accuracy` とは独立した計算になり、突き合わせの意味もあります。
    """)
    return
...
def _():
    mo.md(r"""
    混同行列で、どの数字をどの数字と取り違えたかを見ます。行が正解、列が予測で、
    対角成分が正解数です。対角から外れた位置の数字が、形の似た組み合わせに
    偏っていないかを確認します。
    """)
    return
